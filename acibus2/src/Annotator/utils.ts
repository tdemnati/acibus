...
export const splitWithOffsets = (text: string | any[], offsets: {start: number; end: number}[]) => {
  let lastEnd = 0
  const splits = []
...
export const splitTokensWithOffsets = (text: any[], offsets: {start: number; end: number}[]) => {
  let lastEnd = 0
  const splits = []
